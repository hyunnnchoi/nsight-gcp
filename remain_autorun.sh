...
# 실행하고 싶은 패턴들
PATTERNS=("a7" "a27" "a30" "p8" "p20" "p24" "p28" "p32" "p36")

# 패턴에 해당하는 파일만 선택
JOB_FILES=()
for pattern in "${PATTERNS[@]}"; do
    JOB_FILES+=($(ls ${JOB_DIR}${pattern}*.yaml 2>/dev/null))
done

# 선택된 Job 파일 출력
echo "Selected job files: ${JOB_FILES[@]}"
...
for job_file in "${JOB_FILES[@]}"; do
    ...
    # Job 이름 추출 (a7, p8 등 번호 부분만 가져옴)
    job_number=$(basename "${job_file}" .yaml | grep -oE '^[a-z]+[0-9]+')
