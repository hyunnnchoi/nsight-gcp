...
for job_file in "${JOB_FILES[@]}"; do
    ...
    while true; do
        ...
        if [[ "${job_file}" == a*.yaml ]]; then
            COMPLETED_CONTROLLER=$(kubectl get pods --no-headers | grep "${job_number}" | grep "controller-0" | grep "Completed" | wc -l)
            TOTAL_CONTROLLER=$(kubectl get pods --no-headers | grep "${job_number}" | grep "controller-0" | wc -l)
            ...
            echo "Debug: Completed controller: ${COMPLETED_CONTROLLER} / Total controller: ${TOTAL_CONTROLLER}"
            echo "Debug: Completed workers: ${COMPLETED_WORKERS} / Total workers: ${TOTAL_WORKERS}"

            if [ "${COMPLETED_CONTROLLER}" -eq "${TOTAL_CONTROLLER}" ] && [ "${TOTAL_CONTROLLER}" -gt 0 ]; then
                echo "Controller pod for job ${job_number} is Completed."
                echo "Deleting job: ${job_file}"
                kubectl delete -f "${job_file}"
                break
            elif [ "${COMPLETED_WORKERS}" -eq "${TOTAL_WORKERS}" ] && [ "${TOTAL_WORKERS}" -gt 0 ]; then
